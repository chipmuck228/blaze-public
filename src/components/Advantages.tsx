...
export const Advantages = () => {
  return (
    <section id="advantages" className="bg-[#0f172a] py-24 relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-sky-600/10 rounded-full blur-3xl translate-y-1/3 -translate-x-1/4"></div>
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          <span className="text-[#38bdf8] font-bold tracking-widest uppercase text-sm">The Blaze Difference</span>
          <h2 className="text-3xl md:text-5xl font-bold text-white mt-2 mb-6">Why Choose Blaze Robotics?</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20">
          {features.map((feature, idx) => (
            <div key={idx} className="bg-slate-800/50 p-8 rounded-3xl border border-slate-700 hover:bg-slate-800 transition-colors backdrop-blur-sm">
              <div className="w-12 h-12 bg-[#2563eb] rounded-xl flex items-center justify-center mb-6 shadow-lg shadow-blue-500/20">
                {feature.icon}
              </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 border-t border-slate-800 pt-16">
          {stats.map((stat, idx) => (
            <div key={idx} className="text-center">
              <div className="text-4xl md:text-6xl font-extrabold text-[#38bdf8] mb-2">{stat.value}</div>
              <div className="text-xs md:text-sm uppercase tracking-widest text-slate-500 font-bold">{stat.label}</div>
            </div>
          ))}
        </div>
      </div>
