    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold text-[#0f172a] mb-4 tracking-tight">The Robotics Journey</h2>
          <p className="text-slate-500 max-w-2xl mx-auto text-lg">Your pathway from curiosity to professional engineering starts here.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
          ...
          {journeySteps.map((step, index) => {
            ...
            if (index === 0) {
              ...
              iconTextClass = 'text-[#0f172a]'
            }
            
            return (
              <div 
                key={step.id}
                className="bg-slate-50 p-10 rounded-3xl relative z-10 border border-slate-200 hover:shadow-xl transition-all group"
              >
                ...
                <h3 className="text-2xl font-bold text-[#0f172a] mb-3">{step.stepNumber}: {step.title}</h3>
                <p className="text-slate-500 mb-6">{step.description}</p>
                <Link 
                  href={step.ctaLink} 
                  className="text-[#2563eb] font-bold flex items-center hover:translate-x-2 transition-transform"
                >
